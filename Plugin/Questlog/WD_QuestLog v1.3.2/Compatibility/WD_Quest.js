 ...
 *
 * @command line4
 * @text --- Compatibility with older versions ---
 * @desc Set of obsolete command for compatibility.
 *
 * @command SetCompletionByName
 * @text Set Completion by Name (DON'T USE)
 * @desc This command is here only for compatibility purpouse, USE "SET QUEST COMPLETION PARAMETER"
 *
 * @arg questName
 * @type string
 * @text Quest Name
 * @desc The name of the quest to modify.
 *
 * @arg completion
 * @type boolean
 * @text Completion
 * @desc Whether the quest should be marked as completed (true) or ongoing (false).
 * @default true
 *
 * @command SetCompletionByID
 * @text Set Completion by ID (DON'T USE)
 * @desc This command is here only for compatibility purpouse, USE "SET QUEST COMPLETION PARAMETER"
 *
 * @arg questID
 * @type number
 * @text Quest ID
 * @desc The ID of the quest to modify.
 *
 * @arg completion
 * @type boolean
 * @text Completion
 ...
 * @default false
 *
 * @command RemoveQuestByID
 * @text Remove Quest by ID (DON'T USE)
 * @desc This command is here only for compatibility purpouse, USE "REMOVE QUEST"
 *
 * @arg questID
 * @type number
 * @text Quest ID
 * @desc The ID of the quest to remove.
 *
 * @command RemoveQuest
 * @text Remove Quest by Name(DON'T USE)
 * @desc This command is here only for compatibility purpouse, USE "REMOVE QUEST"
 *
 * @arg questName
 * @text Quest Name
 * @desc Name of the quest to be removed.
 * @type text
 * 
 * @help WD_Quest.js
 *
...
(function() {
  ...
  Window_Title.prototype.constructor = Window_Title;
	...
	PluginManager.registerCommand("WD_Quest", "SetCompletionByName", function(args) {
	  const questName = args.questName;
	  const completion = args.completion === "true" ? "completed" : "ongoing";
	  setQuestCompletionByName(questName, completion);
	  saveQuestList();
	});

	function setQuestCompletionByName(questName, completion) {
	  for (var i = 0; i < questList.length; i++) {
		if (questList[i].name === questName) {
		  questList[i].status = completion;
		  break;
		}
	  }
	}

	PluginManager.registerCommand("WD_Quest", "SetCompletionByID", function(args) {
	  const questID = Number(args.questID);
	  const completion = args.completion === "true" ? "completed" : "ongoing";
	  setQuestCompletionByID(questID, completion);
	  saveQuestList();
	});

	function setQuestCompletionByID(questID, completion) {
	  for (let i = 0; i < questList.length; i++) {
		if (questList[i].id === questID) {
		  questList[i].status = completion;
		  break;
	...
	PluginManager.registerCommand("WD_Quest", "RemoveQuestByID", function(args) {
	  const questID = Number(args.questID);
	  removeQuestByID(questID);
	  saveQuestList();
	});
	...
	PluginManager.registerCommand("WD_Quest", "RemoveQuest", function(args) {
	  const questName = args.questName;
	  removeQuestByName(questName);
	  saveQuestList();
	});
